...
export function ModelPicker({ isOpen, onClose, triggerRef, direction = 'down' }: ModelPickerProps) {
  const [search, setSearch] = useState('')
  const [loading, setLoading] = useState(false)
  const [menuPosition, setMenuPosition] = useState<MenuPosition>({ left: 0, top: 0 })
  const { providers, enabledModelIds, selectedModelId, setSelectedModelId, setProviderModels } = useStore()
  ...
  const allModels = providers
    .flatMap(p => p.models.filter(m => enabledModelIds.includes(m.id)).map(m => ({ ...m, provider: p })))
  
  const filteredModels = search
    ? allModels.filter(m => getDisplayName(m.id).toLowerCase().includes(search.toLowerCase()))
    : allModels
  ...
  const dropdown = (
    ...
    >
      ...
      <div style={{ maxHeight: menuPosition.maxHeight || 360, overflowY: 'auto' }}>
        {loading && (
          <div style={{ padding: '12px 14px', fontSize: '12px', color: 'var(--tx3)' }}>
            Loading...
          </div>
        )}
        {Object.entries(grouped).map(([providerName, models]) => (
          <div key={providerName}>
            <div style={{ padding: '8px 14px 4px', fontSize: 'calc(var(--fs) - 3.5px)', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--tx4)' }}>
              {providerName}
            </div>
            {models.map(m => (
              <button
                ...
                style={{
                  ...
                  padding: '9px 14px',
                  border: 'none',
                  background: selectedModelId === m.id ? 'rgba(var(--acc-r),var(--acc-g),var(--acc-b),.1)' : 'transparent',
                  cursor: 'pointer',
                  transition: 'background var(--ease)',
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = 'rgba(var(--acc-r),var(--acc-g),var(--acc-b),.07)'
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = selectedModelId === m.id ? 'rgba(var(--acc-r),var(--acc-g),var(--acc-b),.1)' : 'transparent'
                }}
              >
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div style={{ fontSize: 'calc(var(--fs) - .5px)', color: 'var(--tx)', marginBottom: '2px' }}>
                    {getDisplayName(m.id)}
                  </div>
                  <div style={{ fontSize: 'calc(var(--fs) - 2.5px)', color: 'var(--tx3)' }}>
                    {formatContext(m.contextWindow)} ctx
                  </div>
                </div>
                {selectedModelId === m.id && (
                  <Check size={16} style={{ color: 'rgba(var(--acc-r),var(--acc-g),var(--acc-b),1)', marginLeft: 8, flexShrink: 0 }} />
                )}
              </button>
            ))}
          </div>
